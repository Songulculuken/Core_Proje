...
namespace BusinessLayer.ValidationRules
{
    ...
    {
        public PortfolioValidator() //Ctor tab tab
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Proje adı boş geçilemez");  //Fluent Validation methodları.İşlem gerçekleşmesi için şartlar
            RuleFor(x => x.ImageUrl).NotEmpty().WithMessage("Görsel alanı boş geçilemez");
            RuleFor(x => x.İmageUrl2).NotEmpty().WithMessage("Görsel2 alanı boş geçilemez");
